    ...
    <div class="row justify-content-center align-items-center"> 
        <div class="card" style="width: 24rem;"> 
            ...
            <div class="card-body"> 
                ...
                @endif 
                <form method="post" enctype="multipart/form-data" action="{{ route('mahasiswa.update', $List->Nim) }}" id="myForm"> 
                @csrf 
                @method('PUT')
                    ...
                    <label for="Kelas">Kelas</label> 
                        <select type="Kelas" name="Kelas" class="form-control" id="Kelas">
                            @foreach ($kelas as $kls)
                                <option value="{{$kls->id}}" {{$List->Kelas_ID == $kls->id ? 'selected' : ' '}}>{{$kls->Nama_Kelas}}</option>
                            @endforeach
                        </select></div>
                <div class="form-group"> 
                ...
                <div class="form-group">
                    <label for="image">Foto</label>
                        <input type="file" class="form-control" required="required" name="foto" value="{{$List->Foto}}">
                        </br>
                        <img width="150px" src="{{asset('storage/'.$List->Foto)}}"></div>
